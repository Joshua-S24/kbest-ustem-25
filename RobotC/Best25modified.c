	...
	while(1 == 1)
  {
  	if (currentState == ACTIVE){
	    ...
	    // Open, close or stop the base
			...
			if (vexRT[Btn6D]){motor[gripServo] = -127;}
			else if (vexRT[Btn5D]){motor[gripServo] = 127;}
			else{motor[gripServo] = 0;}

			if (vexRT[Btn8R] == 1){
				gripping = !gripping;
			}

			if (vexRT[Btn6U] == 1 && keyAngle < 127){
				keyAngle++;
			} else if (vexRT[Btn5U] == 1 && keyAngle > -127){
				keyAngle--;
			}
			motor[keyServo] = keyAngle;
